...
<body>
  ...
  <script>
    ...
    function updateUI(data) {
      ...
      renderPlayers(data.players, me?.id || null, isAdmin);
    ...
    function connectSSE(roomId, playerId) {
      ...
      const params = new URLSearchParams({ roomId, playerId: playerId || '' });
    ...
    async function joinRoom(roomId, name, asAdmin = false) {
      localStorage.setItem('amongus_name', name);
      const payload = { roomId, name };
      if (asAdmin && state.adminToken) payload.adminToken = state.adminToken;
      const result = await api('/api/join', payload);
      state.playerId = result.playerId;
      state.roomId = result.roomId;
      if (result.adminToken) state.adminToken = result.adminToken;
    ...
    el('startBtn').addEventListener('click', async () => {
      try { await startGame(); } catch (err) { setMessage(err.message, 'error'); }
    });

    el('endMeetingBtn').addEventListener('click', async () => {
      try { await endMeeting(); } catch (err) { setMessage(err.message, 'error'); }
    });
    el('endMeetingBtnSide').addEventListener('click', async () => {
      try { await endMeeting(); } catch (err) { setMessage(err.message, 'error'); }
    });
    el('meetingBtn').addEventListener('click', async () => {
      try { await triggerMeeting(); } catch (err) { setMessage(err.message, 'error'); }
    });

    el('copyRoomBtn').addEventListener('click', () => {
      const code = state.current?.roomId;
    ...
    // Auto-join from URL when possible.
    (async function init() {
...
server.listen(PORT, () => {
  console.log(`Among Us IRL app running on http://localhost:${PORT}`);
});
